...
export type UsernameMatch='EXACT USERNAME MATCH'|'STRONG POSSIBLE MATCH'|'POSSIBLE MATCH'|'LOW CONFIDENCE'|'UNVERIFIED';
export interface UsernameProfile {id:string;platform:string;profileUrl:string;username:string;displayName:string;bio:string;publicWebsite?:string;publicLocation?:string;publicDescription:string;publicEmail?:string;organization?:string;lastObserved:string;source:string;confidence:number;match:UsernameMatch;signals:{label:string;score:number;detail:string}[];verification:'UNVERIFIED'|'VERIFIED'|'DISPUTED'}
export interface UsernameComparison {left:string;right:string;usernameSimilarity:number;displayNameSimilarity:number;bioSimilarity:number;linkedDomain:string[];publicEmail:string[];locations:string[];commonReferences:string[];score:number;conclusion:'LIKELY SAME DIGITAL IDENTITY'|'INSUFFICIENT EVIDENCE';inference:true}
export interface UsernameIntelligenceResult extends Omit<FootprintResult,'kind'|'targetType'> {kind:'username';targetType:'username';normalizedUsername:string;mode:'lookup'|'footprint'|'web'|'graph'|'timeline'|'pivot'|'compare';profiles:UsernameProfile[];comparison?:UsernameComparison;mocked:boolean;provider:string;disclaimer:string}
export type IntelligenceResult=FootprintResult|PhoneIntelligenceResult|UsernameIntelligenceResult;
